...
public class Segmento {
    ...
    public boolean onSegment(Ponto c, Ponto a, Ponto b) {
        if (!colinear(a, b, c)) {
            return false; // Se não forem colineares, `c` não pode estar no segmento
        }
    ...
    /**
     * Verifica se três pontos são colineares usando a área do triângulo formado por eles.
     * Se a área for zero, os pontos estão na mesma linha reta.
     */
    ...
    public int intersection(Circulo c) {
        ...
        if (discriminante > 0)
            return 1;  // Interseção real
        else
            return 0;  // Não há interseção ou tangência
    }
